...
use crate::theme::t;
use crate::tiles::{key_event_bytes, TerminalSession};
...
impl GameKind {
    ...
    pub fn subtitle(self) -> &'static str {
        match self {
            ...
            Self::SpaceInvaders => "3D space flight with parallax planets",
            Self::Penguin3D => "voxel penguin collector with 3D lighting",
        }
    }

    /// Relative path from the asciivision project root to the OpenTUI game script.
    fn opentui_script(self) -> &'static str {
        match self {
            Self::PacMan => "games/PacMan/index.ts",
            Self::SpaceInvaders => "games/Space-Invaders/index.ts",
            Self::Penguin3D => "games/3d-penguin/index.ts",
        }
    }
...
impl GamesPanel {
    ...
    pub fn tick(&mut self, dt: f32) {
        if let Some(session) = &mut self.session {
            ...
            if session.is_finished() {
                let label = session.kind().label().to_string();
                self.session = None;
                self.status = format!("games: {} exited", label);
            }
        }
    }
    ...
    pub fn handle_key(&mut self, key: KeyEvent) -> bool {
        ...
        if let Some(session) = &mut self.session {
            match key.code {
                ...
                KeyCode::Char('r') | KeyCode::Char('R') if !session.is_opentui() => {
                    session.restart();
    ...
    pub fn render(&mut self, frame: &mut Frame, area: Rect, phase: f32, is_focused: bool) {
    ...
    fn render_menu(&mut self, frame: &mut Frame, area: Rect, phase: f32) {
        let spinner = ["|", "/", "-", "\\"][((phase * 7.0) as usize) % 4];
        let opentui_ok = find_bun().is_ok();
        let engine_tag = if opentui_ok {
            Span::styled(
                " [OpenTUI 3D]",
        ...
        for (idx, game) in GameKind::ALL.iter().enumerate() {
            ...
            let has_script = find_game_script(*game).is_ok() && opentui_ok;
            let badge = if has_script { " [3D]" } else { "" };
            lines.push(Line::from(vec![
                ...
                Span::styled(badge, Style::default().fg(Color::Rgb(34, 211, 238))),
            ]));
        }
...
enum GameSession {
    ...
    OpenTui(TerminalSession, GameKind),
}

impl GameSession {
    fn new_builtin(kind: GameKind) -> Self {
        match kind {
    ...
    fn kind(&self) -> GameKind {
        match self {
            ...
            Self::OpenTui(_, kind) => *kind,
        }
    }

    fn is_opentui(&self) -> bool {
        matches!(self, Self::OpenTui(..))
    }
    ...
    fn tick(&mut self, _dt: f32) {
        match self {
            Self::PacMan(game) => game.tick(_dt),
            Self::SpaceInvaders(game) => game.tick(_dt),
            Self::Penguin3D(game) => game.tick(_dt),
            Self::OpenTui(..) => {
                // OpenTUI games run their own event loop in the PTY process.
                // No tick needed — they render at their own 30 FPS.
            }
        }
    }
    ...
    fn render(&mut self, buffer: &mut Buffer, area: Rect) {
        match self {
            ...
            Self::OpenTui(session, _) => {
                session.render(buffer, area, true);
            }
        }
    }
    ...
    fn is_finished(&self) -> bool {
        match self {
            Self::OpenTui(session, _) => session.has_exited(),
            _ => false,
        }
    }
}

/// Try to find bun and the OpenTUI game script, then launch it in a PTY.
fn try_launch_opentui(kind: GameKind) -> anyhow::Result<TerminalSession> {
    // Find the bun binary
    let bun = find_bun()?;

    // Find the game script relative to the asciivision project root
    let script = find_game_script(kind)?;
    let game_dir = script
        .parent()
...
fn find_bun() -> anyhow::Result<PathBuf> {
    ...
    Err(anyhow::anyhow!("bun not found"))
}

/// Locate the OpenTUI game script, searching several candidate directories.
fn find_game_script(kind: GameKind) -> anyhow::Result<PathBuf> {
    let script_rel = kind.opentui_script();

    let mut candidates = Vec::new();

    // 1. Relative to current working directory
    if let Ok(cwd) = std::env::current_dir() {
        candidates.push(cwd.join(script_rel));
    }
    ...
    // 3. Relative to CARGO_MANIFEST_DIR at compile time
    let manifest_dir = PathBuf::from(env!("CARGO_MANIFEST_DIR"));
    candidates.push(manifest_dir.join(script_rel));
