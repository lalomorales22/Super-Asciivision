...
impl TerminalSession {
    /// Spawn a shell terminal session (used by the Tiles panel).
    ...
    /// Spawn a terminal session running an arbitrary command (used by Games panel for OpenTUI).
    pub fn spawn_command(program: &str, args: &[&str], cwd: Option<&std::path::Path>) -> Result<Self> {
        let pty_system = native_pty_system();
        let pair = pty_system
            .openpty(PtySize {
                rows: 40,
            ...
            .context("failed to allocate PTY for game")?;
        ...
        let parser = Arc::new(Mutex::new(vt100::Parser::new(40, 100, SCROLLBACK_LINES)));
        let parser_reader = Arc::clone(&parser);
        let mut reader = pair
            .master
            .try_clone_reader()
            .context("failed to clone PTY reader for game")?;
        thread::spawn(move || {
            let mut buf = [0_u8; 8192];
            loop {
                match reader.read(&mut buf) {
        ...
        let writer = Arc::new(Mutex::new(
            pair.master
                .take_writer()
                .context("failed to acquire PTY writer for game")?,
        ));
    ...
    pub fn render(&mut self, buffer: &mut Buffer, area: Rect, is_active: bool) {
...
pub fn map_color(color: vt100::Color, fallback: Color) -> Color {
    match color {
...
pub fn key_event_bytes(key: KeyEvent, application_cursor: bool) -> Vec<u8> {
    match key.code {
