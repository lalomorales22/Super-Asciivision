...
const SLASH_COMMANDS: &[(&str, &str)] = &[
    ...
    ("/3d", "toggle 3D fx"),
    ("/effects", "toggle 3D fx"),
    ("/fx", "cycle through 3D effects"),
    ("/randomize", "randomize theme palette"),
    ("/theme random", "randomize theme palette"),
    ("/theme reset", "reset theme to default"),
    ("/theme default", "reset theme to default"),
    ("/analytics", "show analytics in focused tile"),
    ("/layout", "cycle layout preset"),
    ("/layout ", "set layout: default, dual, triple, quad, webcam, focus"),
    ("/sysmon", "show system monitor in focused tile"),
    ("/games", "load the arcade panel"),
    ("/games ", "launch specific game (pacman, space, penguin)"),
    ("/tiles", "boot the Tiles panel"),
    ("/tiles ", "boot the Tiles panel with specific terminal count (1-8)"),
    ("/server ", "spin up local video chat server"),
    ("/connect ", "connect to remote video chat server"),
    ("/chat ", "send chat message over video socket"),
    ("/username ", "set your user handle"),
    ("/provider ", "switch AI route (claude, grok, gpt5, gemini, ollama)"),
    ("/ollama", "open ollama local model picker"),
    ("/trust", "cycle command execution trust policy"),
    ("/remember ", "store fact in agent memory (key=val)"),
    ("/forget ", "remove fact from agent memory"),
    ("/recall ", "look up fact in agent memory"),
    ("/memory", "dump full agent memory context"),
    ("/pin", "pin current prompt to sysmon"),
    ("/unpin", "remove pinned prompt"),
    ("/stream", "toggle AI streaming text"),
    ("/run ", "run shell operation"),
    ("/bash ", "run shell operation"),
    ("/curl ", "run curl command"),
    ("/brew ", "run brew command"),
];
...
impl App {
        ...
        while let Ok(event) = self.events_rx.try_recv() {
            match event {
                ...
                AppEvent::YoutubeReady { title, source } => {
                    ...
                    let (yt_cols, yt_rows) = crossterm::terminal::size().unwrap_or((132, 46));
                    let yt_decode_cols = yt_cols.saturating_sub(6).max(40);
                    let yt_decode_rows = yt_rows.saturating_sub(8).max(20);
                    match VideoPlayer::new(source, (yt_decode_cols, yt_decode_rows), false) {
                        Ok(player) => {
        ...
        match key.code {
            ...
            KeyCode::Enter => {
                ...
                if !self.get_autocomplete_suggestions().is_empty() {
                    applied = self.apply_autocomplete();
                }
                if !applied {
    ...
    fn get_autocomplete_suggestions(&self) -> Vec<&'static (&'static str, &'static str)> {
        if self.input.starts_with('/') && !self.input.trim().contains(' ') {
            let limit = 8;
            SLASH_COMMANDS
                .iter()
    ...
    fn apply_autocomplete(&mut self) -> bool {
        let suggestions = self.get_autocomplete_suggestions();
        if !suggestions.is_empty() {
            let idx = self.autocomplete_index.min(suggestions.len().saturating_sub(1));
            let chosen = suggestions[idx].0.to_string();
            if self.input == chosen {
                return false;
            }
            self.input = chosen;
            self.autocomplete_index = 0;
    ...
    fn render_video_panel(&self, frame: &mut Frame, area: Rect, phase: f32) {
        ...
        if self.video_enabled {
            if let Some(video) = &self.video {
                ...
                render_gradient_text_clipped(frame.buffer_mut(), inner.x + 1, inner.y, &meta, t().accent4, t().text, Some(inner.x + inner.width));
                return;
            }
        }
    ...
    fn render_input(&self, frame: &mut Frame, area: Rect) {
        ...
        if !suggestions.is_empty() {
            ...
            let items: Vec<ratatui::widgets::ListItem> = suggestions
                .iter()
                .enumerate()
                .map(|(i, (cmd, desc))| {
                    let text = format!("{:<20} {}", cmd, desc);
                    if i == self.autocomplete_index {
                        ratatui::widgets::ListItem::new(text).style(Style::default().bg(t().accent1).fg(t().bg_base).bold())
                    } else {
                        ratatui::widgets::ListItem::new(text).style(Style::default().fg(t().text))
                    }
                })
                .collect();
...
fn render_gradient_text(buffer: &mut Buffer, x: u16, y: u16, text: &str, start: Color, end: Color) {
    render_gradient_text_clipped(buffer, x, y, text, start, end, None);
}

fn render_gradient_text_clipped(buffer: &mut Buffer, x: u16, y: u16, text: &str, start: Color, end: Color, right_edge: Option<u16>) {
    let buf_area = *buffer.area();
    if y < buf_area.y || y >= buf_area.y + buf_area.height {
        return;
    }
    let max_x = right_edge.unwrap_or(buf_area.x + buf_area.width);
